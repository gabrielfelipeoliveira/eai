...
- Lucas Reiter: `EAI-072` em andamento como epic tecnico da reestruturacao UX; `EAI-077` em andamento para pipeline existente.
- Gabriel Felipe Ferreira de Oliveira: nenhum card ativo conhecido no Trello.

Branches atuais:

```text
Lucas Reiter: `feature/eai-077-pipeline` derivada de `epic/eai-072-ux-restructure`.
...
- Implementar `EAI-077`, reestruturando visualmente o Pipeline existente sem alterar etapas, acoes, filtros, regras de negocio ou APIs.
