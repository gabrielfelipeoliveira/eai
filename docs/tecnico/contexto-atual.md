...
- Lucas Reiter: `EAI-072` em andamento como epic tecnico da reestruturacao UX; `EAI-078` em andamento para conversas existentes.
- Gabriel Felipe Ferreira de Oliveira: nenhum card ativo conhecido no Trello.

Branches atuais:

```text
Lucas Reiter: `feature/eai-078-conversas` derivada de `epic/eai-072-ux-restructure`.
...
- Implementar `EAI-078`, reestruturando visualmente Conversas existentes sem alterar mensagens, anexos, status, regras de negocio ou APIs.
