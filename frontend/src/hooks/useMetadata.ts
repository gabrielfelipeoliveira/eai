...
export const fallbackMetadata: MetadataCatalog = {
  ...
  leadStatuses: [
    ...
    option('SIMULATING', 'lead.status.simulating', 'Simulacao', 7, 'info'),
    option('PROPOSAL_APPROVED', 'lead.status.proposal_approved', 'Proposta aprovada', 8, 'secondary'),
    option('PROPOSAL_SENT', 'lead.status.proposal_sent', 'Proposta enviada', 9, 'secondary'),
    option('SOLD', 'lead.status.sold', 'Vendido', 10, 'success'),
    option('LOST', 'lead.status.lost', 'Perdido', 11, 'error'),
    option('DUPLICATED', 'lead.status.duplicated', 'Duplicado', 12, 'default'),
  ],
  leadSources: [
